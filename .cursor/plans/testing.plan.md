...
1. **创建 src/lib/trading/rules.test.ts**
...
1. **创建 src/lib/trading/engine.test.ts**
...
1. **创建 src/lib/data/sina-api.test.ts**
...
1. **创建 src/lib/data/cache.test.ts**
...
1. **创建 src/lib/ai/decision.test.ts**
  - getAIDecision() 测试
    - 成功获取AI决策
    - 解析JSON响应正确
    - API失败时抛出异常
...
1. **创建 src/lib/backtest/runner.test.ts**
...
1. **创建 src/lib/db/queries.test.ts**
  - 使用内存SQLite测试
    - saveTrade() 测试
    - 保存交易记录
    - 返回保存的记录
...
1. **创建 src/app/api/stock/data/route.test.ts**
  - GET 请求测试
    - 返回股票数据
    - 参数验证
    - 错误处理
2. **创建 src/app/api/backtest/run/route.test.ts**
  - POST 请求测试
    - 启动回测
    - 参数验证
    - 返回回测结果
3. **创建 src/app/api/history/positions/route.test.ts**
  - GET 请求测试
    - 返回持仓历史
    - 分页参数处理
...
1. **创建 src/tests/integration/backtest-flow.test.ts**
  - 完整回测流程测试
    - 从获取数据到执行交易
    - 验证最终账户状态
    - 验证交易记录完整性
